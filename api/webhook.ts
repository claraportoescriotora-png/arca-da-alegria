...
async function sendWelcomeEmail(email: string, password: string, magicLink: string) {
    ...
    try {
        ...
        if (orderStatus === 'paid' || eventType === 'order_approved' || eventType === 'subscription_renewed') {
            ...
            if (!userId) {
                ...
                console.log(`Creating user record for ${email} with password`);
                ...
                if (createError) {
                    ...
                    if (linkData?.properties?.action_link) {
                        try {
                            await sendWelcomeEmail(email, tempPassword, linkData.properties.action_link);
                        } catch (e) {
            ...
            const isChargeback = orderStatus === 'chargedback';
            const newStatus = isChargeback ? 'blocked' : 'canceled';
            
            console.log(`Processing ${orderStatus} for ${email}. New status: ${newStatus}`);
            ...
            // 2. If it was a specific product, remove its access
            const productKey = fullUrl.searchParams.get('p') || payload.key;
            if (productKey) {
                const { data: product } = await supabase
                    .from('products')
                ...
                if (product) {
                    ...
                    if (profile) {
                        console.log(`Removing product access for ${productKey} from ${profile.id}`);
                        await supabase.from('user_products')
                            .delete()
                            .match({ user_id: profile.id, product_id: product.id });
                    }
                }
            }

            // 3. If it's a chargeback, remove ALL products just to be safe
            if (isChargeback) {
                 const { data: profile } = await supabase
                        .from('profiles')
                        .select('id')
                ...
                if (profile) {
                    console.log(`Fraud detected (chargeback). Purging all products for user ${profile.id}`);
                    await supabase.from('user_products')
                        .delete()
            ...
            // 4. Update CRM Status (Simple update, the stage is enough or we could fetch and append tags)
            try {
                await supabase.from('crm_leads').update({
                    stage: 'Perdido',
                    updated_at: new Date().toISOString()
                }).eq('email', email);
            } catch (e) { /* ignore crm update errors */ }
        }
